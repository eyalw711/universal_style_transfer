...
These visual results show that our models are not sufficiently trained. The main difficulty in training these models was a lack of compute power. The models, based on VGG-19 are relatively large and converge very slowly towards optimality. The training process of our models used a Tesla K80 GPU offered on Google Colab, and took three weeks in which each model was trained numerous epochs on the COCO test and validation subsets. In hindsight this wasn't enough for VGG-19 based models to achieve proper results. We strongly believe that given more time and powerful resources we could have trained better performing decoders.\\
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.4\linewidth}
		...
		%\caption{Boost UST}
	\end{subfigure}
	\caption{First Boosted UST results compared to UST. Top left is the content image, top right is the style image, bottom left is the regular UST, bottom right is the UST+boost}
	\label{fig:Boost1}
\end{figure}
\begin{figure}[H]
	\centering
	...
	\begin{subfigure}[b]{0.4\linewidth}
		...
		%\caption{Boost UST}
	\end{subfigure}
	\caption{Second Boosted UST results compared to UST. Top left is the content image, top right is the style image, bottom left is the regular UST, bottom right is the UST+boost}
	\label{fig:Boost2}
\end{figure}
